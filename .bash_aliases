...
RDBG='\033[0;41m' # RedBG
GRBG='\033[0;42m' # RedBG
NC='\033[0m' # No Color
...
#* Apache
alias aprs='sudo systemctl restart apache2'
alias aprl='sudo systemctl reload apache2'
...
# Create a tag in git, then use that to set version information for Drupal's environment indicators.
drt () {
  echo "Setting git tag...\n"
...
# Shorthand for terminus remote:drush
trd () {
  if [ -v SITE ]; then
  echo "${GRBG} $SITE ${NC}: select Pantheon server:"
    select env in dev test live Quit
    case $env in
    "dev"|"test"|"live")
...
proddump () {
  scp root@$prod:/data/db-dumps/${1} /home/jesse/db-dumps/.;
}

proddump () {
  scp root@$dev:/data/db-dumps/${1} /home/jesse/db-dumps/.;
}
...
alias chownsites='sudo chown -R jesse:jesse /home/jesse/Sites';
